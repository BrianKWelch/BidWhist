...
import { DollarSign, Search, User, Phone, MapPin, ChevronDown, ChevronRight } from 'lucide-react';
...
interface IndividualPlayerPaymentsPageProps {
  playerId?: string;
  onBackToCommandCenter?: () => void;
}

const IndividualPlayerPaymentsPage: React.FC<IndividualPlayerPaymentsPageProps> = ({ playerId, onBackToCommandCenter }) => {
  ...
  const players: Player[] = teams.flatMap(team => [
    {
      id: team.player1_id, // Use actual player ID
    ...
    {
      id: team.player2_id, // Use actual player ID
  ...
  // Filter to show only the specific player if playerId is provided
  const filteredPlayers = playerId ? players.filter(player => player.id === playerId) : players;
  ...
  const playersByTournament = tournaments.reduce((acc, tournament) => {
    const tournamentPlayers = searchFilteredPlayers.filter(player => {
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="flex items-center gap-2">
              ...
              {playerId ? 'Player Payment Details' : `Individual Player Payments (${players.length} players)`}
            </CardTitle>
          ...
          <div className="space-y-4">
            ...
            {players.length === 0 ? (
              ...
              <div className="space-y-4">
                   ...
                   const isOpen = playerId ? true : (searchTerm.trim() ? tournamentPlayers.length > 0 : openTournaments.has(tournamentId));
                  return (
...
export default IndividualPlayerPaymentsPage;
