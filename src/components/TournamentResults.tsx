...
export const TournamentResults: React.FC<TournamentResultsProps> = ({ tournamentId }) => {
  const { tournaments, teams, schedules, games, clearTournamentResults, submitGame, scoreSubmissions, resetAllTournamentData } = useAppContext();
        ...
        for (let round = 1; round <= numRounds; round++) {
          ...
          const matchTeamIds = [match.teamA, match.teamB]
            .map(id => {
              ...
              if (typeof id === 'object' && id !== null && 'id' in id && (id as any).id != null) return typeof (id as any).id === 'string' ? (id as any).id : '';
              return id;
            })
            .filter(Boolean)
            .sort();
  ...
  const handleResetTableDisplay = () => {
    ...
    window.alert('Table display reset to 0s and blanks.');
  };
        ...
        <div className="flex items-center gap-4 justify-between">
          ...
          <div className="flex gap-2">
            <button
              ...
              onClick={() => {
                if (window.confirm('Are you sure you want to reset ALL tournament data? This will clear all games, schedules, results, and start fresh with preloaded teams.')) {
                  if (typeof resetAllTournamentData === 'function') {
                    resetAllTournamentData();
                  } else if (typeof clearTournamentResults === 'function') {
                    clearTournamentResults(effectiveTournamentId);
                  }
                  window.alert('All tournament data has been reset.');
                }
              }}
            >
