...
const ScoreConfirmation = ({ team, match, onComplete }: { team: Team; match: any; onComplete: () => void }) => {
 ...
 const PlayerPortalFixed = () => {
  ...
  const portalActiveTournament = getActiveTournament();
  const isMaltTournament = portalActiveTournament?.rotationType === 'malt';
  const maltSchedule = portalActiveTournament ? schedules.find(s => s.tournamentId === portalActiveTournament.id) : null;
  const maltNumTables = maltSchedule ? maltSchedule.matches.filter(m => m.round === 1 && !m.isBye).length : 0;
  const maltNumRounds = isMaltTournament ? (portalActiveTournament?.maltRounds || 0) : 0;

  // Build projected future rounds for MALT tournaments (rounds not yet generated by admin)
  const maltProjectedRounds: any[] = [];
  if (isMaltTournament && maltNumTables > 0 && team && maltNumRounds > 0) {
    const generatedRounds = new Set(teamSchedule.map((m: any) => m.round));
    for (let round = 2; round <= maltNumRounds; round++) {
      if (generatedRounds.has(round)) continue;

      // Only project table/opponent from the most recent confirmed scheduled match
      const prevMatch = teamSchedule.find((m: any) => m.round === round - 1 && !m.isBye && m.gameResult);
      let projectedTable: number | null = null;
      let opponentTeamObj: any = null;

      if (prevMatch?.table && prevMatch.gameResult) {
        const { winner: wt, loser: lt } = getMaltNext(maltNumTables, prevMatch.table);
        projectedTable = prevMatch.gameResult.isWin ? wt : lt;

        // Find opponent from partner table (tables pair: 1&2, 3&4, ...)
        const partnerTable = prevMatch.table % 2 === 0 ? prevMatch.table - 1 : prevMatch.table + 1;
        const partnerSchedMatch = maltSchedule?.matches.find(
          (m: any) => m.round === round - 1 && m.table === partnerTable
        );
        if (partnerSchedMatch) {
          ...
          if (partnerGame) {
            const partnerId = prevMatch.gameResult.isWin
              ? (partnerGame.winner === 'teamA' ? String(partnerGame.teamA) : String(partnerGame.teamB))
              : (partnerGame.winner === 'teamA' ? String(partnerGame.teamB) : String(partnerGame.teamA));
            opponentTeamObj = teams.find((t: any) => String(t.id) === partnerId) || null;
          }
        }
      }

      maltProjectedRounds.push({
        round,
        table: projectedTable,
        opponentTeam: opponentTeamObj,
        opponentId: opponentTeamObj?.id || null,
        cardType: 'projected',
        isProjected: true,
        isBye: false,
        gameResult: null,
        status: '',
        id: `projected-r${round}`
      });
    }
  }

  const displaySchedule = isMaltTournament ? [...teamSchedule, ...maltProjectedRounds] : teamSchedule;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto p-4" style={{ paddingTop: activeMessage ? 'calc(18rem + 60px)' : '18rem' }}>
        ...
        <div className="mb-6">
          <div className="text-center mb-4">
            ...
            {isMaltTournament && maltNumTables > 0 && (
              <button
                className="mt-3 inline-flex items-center gap-1.5 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold rounded-lg shadow"
                onClick={() => setShowRotationChart(true)}
              >
                📊 View {maltNumTables}-Table Rotation Chart
              </button>
            )}
          </div>
        </div>
          ...
          <CardContent>
            {(() => {
              ...
              <div className="space-y-6">
                {/* Show all rounds */}
                {(() => {
                  const maxRound = Math.max(...displaySchedule.map((m: any) => m.round));
                  return Array.from({ length: maxRound }, (_, i) => i + 1).map(round => {
                    const roundMatches = displaySchedule.filter((match: any) => match.round === round);
                    return (
                      <div key={round} className="space-y-3">
                        <div className="space-y-3">
                          {roundMatches.map((match: any, index: number) => {
                            ...
                            } else if (match.cardType === 'projected') {
                              cardClassName += 'bg-gray-50 border-gray-200 border-dashed opacity-75';
                            } else {
                                ...
                                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2">
                                  <div className="flex-1">
                                    ...
                                    {match.isBye ? (
                                      ...
                                      <div>
                                        {/* Projected round (MALT - not yet generated) */}
                                        {match.cardType === 'projected' && (
                                          <div className="flex items-center justify-between h-16">
                                            ...
                                            <div className="flex flex-col items-center justify-center">
                                              ...
                                              {match.opponentTeam ? (
                                                <>
                                                  <span className="font-bold text-lg text-gray-700">{match.opponentTeam.id}</span>
                                                  <span className="text-xs text-gray-500">
                                                    {match.opponentTeam.player1FirstName}/{match.opponentTeam.player2FirstName}
                                                  </span>
                                                </>
                                              ) : (
                                                <span className="font-bold text-lg text-gray-400">TBD</span>
                                              )}
                                            </div>
                                            {match.table ? (
                                              ...
                                              <Badge variant="outline" className="text-xs text-gray-400 border-gray-300 border-dashed">
                                                Table TBD
                                              </Badge>
                                            )}
                                          </div>
                                        ...
                                        {match.cardType === 'completed' && match.gameResult && (
                                          <>
                                            ...
                                            {isMaltTournament && maltNumTables > 0 && match.table && match.gameResult && (() => {
                                              // Prefer the actual scheduled next match if admin has generated it
                                              const nextMatch = teamSchedule.find((m: any) => m.round === match.round + 1 && !m.isBye);
                                              if (nextMatch) {
                                                return (
                                                  <div className="text-center mt-2">
                                                    <Badge variant="outline" className="border-blue-400 text-blue-700 text-xs font-semibold">
                                                      Go to Table {nextMatch.table}
                                                    </Badge>
                                                  </div>
                                                );
                                              }
                                              // Otherwise compute from MALT formula
                                              const t = match.table;
                                              const N = maltNumTables;
                                              const loser = Math.ceil(t / 2);
                                              const winner = N % 2 === 0
                                                ? N / 2 + Math.ceil(t / 2)
                                                : (N + 1) / 2 + Math.ceil((t - 1) / 2);
                                              const nextTable = match.gameResult.isWin ? winner : loser;
                                              return (
                                                <div className="text-center mt-2">
                                                  <Badge variant="outline" className="border-blue-400 text-blue-700 text-xs font-semibold">
                                                    Go to Table {nextTable}
                                                  </Badge>
                                                </div>
                                              );
                                            })()}
      ...
      {/* MALT Rotation Chart Modal */}
      {showRotationChart && maltNumTables > 0 && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-sm w-full mx-4 shadow-xl">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-bold">{maltNumTables}-Table Rotation</h3>
              <button
                className="text-gray-500 hover:text-black text-xl font-bold"
                onClick={() => setShowRotationChart(false)}
              >
                ×
              </button>
            </div>
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="border-b-2 border-gray-200">
                  <th className="py-1 text-center">Table</th>
                  <th className="py-1 text-center text-green-700">Winner goes to</th>
                  <th className="py-1 text-center text-red-700">Loser goes to</th>
                </tr>
              </thead>
              <tbody>
                {Array.from({ length: maltNumTables }, (_, i) => {
                  const t = i + 1;
                  const loser = Math.ceil(t / 2);
                  const winner = maltNumTables % 2 === 0
                    ? maltNumTables / 2 + Math.ceil(t / 2)
                    : (maltNumTables + 1) / 2 + Math.ceil((t - 1) / 2);
                  return (
                    <tr key={t} className={t % 2 === 0 ? 'bg-gray-50' : ''}>
                      <td className="py-1 text-center font-medium">{t}</td>
                      <td className="py-1 text-center text-green-700 font-semibold">{winner}</td>
                      <td className="py-1 text-center text-red-700 font-semibold">{loser}</td>
                    </tr>
                  );
                })}
              </tbody>
